...
BOOL OpsecCheck(PSECURITY_DESCRIPTOR pSD, BOOL bSaclPresent, PACL pSACL, LPCWSTR object, BOOL verbose, BOOL opsec) {
	...
	BOOL CheckSACLForFile(LPCWSTR path, BOOL isSingleCheck, BOOL verbose, BOOL opsec) {
		...
		DWORD result = GetNamedSecurityInfo(path, SE_FILE_OBJECT, SACL_SECURITY_INFORMATION, NULL, NULL, NULL, &pSACL, &pSD);
		...
		if (result == ERROR_SUCCESS) {  // Check if the SACL was retrieved successfully
	...
	BOOL CheckSACLForRegistryKey(HKEY hKey, LPCWSTR subKey, BOOL isSingleCheck, BOOL verbose, BOOL opsec) {
		...
		DWORD result = RegGetKeySecurity(hKey, SACL_SECURITY_INFORMATION, NULL, &dwSDSize);
		...
		if (result == ERROR_INSUFFICIENT_BUFFER) {
			...
			result = RegGetKeySecurity(hKey, SACL_SECURITY_INFORMATION, pSD, &dwSDSize);
		}
		...
		do {
			...
			if (wcscmp(findFileData.cFileName, L".") != 0 && wcscmp(findFileData.cFileName, L"..") != 0) {
				...
				if (findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
					...
					CheckSACLForFile(fullPath, FALSE, verbose, opsec);
				}
			}
	...
	void EnumerateFilesInDirectory(LPCWSTR directory, BOOL opsec, BOOL verbose) {
		...
		do {
			if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
				WCHAR fullPath[MAX_PATH];
				swprintf(fullPath, MAX_PATH, L"%s\\%s", directory, findFileData.cFileName);
				CheckSACLForFile(fullPath, TRUE, verbose, opsec);
			}
		} while (FindNextFile(hFind, &findFileData) != 0);
	...
	int wmain(int argc, wchar_t* argv[]) {
		...
		if (registryMode || serviceMode || fileMode || directoryMode) {
		...
		if (registryMode) {
			...
			CheckSACLForFile(fileName, TRUE, verboseMode, opsec);
